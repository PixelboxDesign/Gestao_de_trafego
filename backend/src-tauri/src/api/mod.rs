...
pub async fn start_server(state: Arc<Mutex<AppState>>) {
    ...
    let app = Router::new()
        ...
        .route("/api/catalogo/v2/kit/:marca/:nome/componentes", axum::routing::put(catalogo_db::atualizar_componentes_kit))
        .route("/api/catalogo/criar-produto", axum::routing::post(catalogo_db::criar_produto))
        .route("/api/catalogo/criar-kit", axum::routing::post(catalogo_db::criar_kit))
