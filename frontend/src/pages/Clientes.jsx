...
function ModalPedidos({ cliente, onClose }) {
  ...
  const [pedidoAberto, setPedidoAberto] = useState(null); // id do pedido expandido
    ...
    >
      ...
      >
        ...
        <div style={{
          ...
          <button
            onClick={onClose}
            style={{ background: 'none', border: 'none', cursor: 'pointer', padding: '0.25rem', borderRadius: 6, color: 'var(--gray)' }}
            title="Fechar (ESC)"
          >
            <X size={20} />
          </button>
        ...
        <div style={{ overflowY: 'auto', flex: 1, padding: '1rem 1.5rem' }}>
          ...
          {loading && (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '3rem', gap: '0.75rem', color: 'var(--gray)' }}>
              <div className="spinner" /> Buscando pedidos e produtos...
            </div>
          )}

          {erro && (
            <div style={{ background: '#fdecea', border: '1px solid #f44336', borderRadius: 8, padding: '1rem', display: 'flex', gap: '0.75rem' }}>
              <AlertCircle size={20} color="#c62828" style={{ flexShrink: 0 }} />
              <span style={{ color: '#c62828', fontSize: '0.9rem' }}>{erro}</span>
            </div>
          )}
                  ...
                  {dados.pedidos.map((pedido) => {
                    ...
                    return (
                      <div key={chave} style={{ border: '1px solid var(--border)', borderRadius: 8, overflow: 'hidden' }}>
                        {/* Linha do pedido — clicável para expandir */}
                        <div
                          onClick={() => setPedidoAberto(aberto ? null : chave)}
                          style={{
                            display: 'grid',
                            gridTemplateColumns: '1fr auto auto auto auto',
                            gap: '1rem', alignItems: 'center',
                            padding: '0.75rem 1rem', cursor: 'pointer',
                            background: aberto ? '#f0f4ff' : '#fff',
                        ...
                        >
                          <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                            ...
                            <span style={{
                              ...
                              background: pedido.origem === 'E-commerce' ? 'rgba(33,150,243,0.12)' : 'rgba(76,175,80,0.12)',
                              color:      pedido.origem === 'E-commerce' ? '#1565c0'               : '#388e3c',
                            }}>
                              {pedido.origem}
                            </span>
                          </div>
                          <span style={{ fontSize: '0.82rem', color: 'var(--gray)', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
                            <Calendar size={13} /> {fmtData(pedido.pedido_data)}
                          </span>
                          ...
                          <span style={{ fontSize: '0.875rem', fontWeight: 700, color: 'var(--success)' }}>
                            R$ {fmtReal(pedido.pedido_total)}
                          </span>
                          <span style={{ fontSize: '1rem', color: 'var(--gray)', userSelect: 'none' }}>
                            {aberto ? '▲' : '▼'}
                          </span>
                        </div>
                            ...
                            <table style={{ width: '100%', fontSize: '0.82rem', borderCollapse: 'collapse' }}>
                              <thead>
                                <tr style={{ background: '#f0f0f0' }}>
                                  ...
                                  <th style={{ padding: '0.5rem 0.75rem', textAlign: 'center', fontWeight: 600, whiteSpace: 'nowrap' }}>Qtd</th>
                                  <th style={{ padding: '0.5rem 0.75rem', textAlign: 'right', fontWeight: 600, whiteSpace: 'nowrap' }}>Vlr Unit.</th>
                                  <th style={{ padding: '0.5rem 1rem',    textAlign: 'right', fontWeight: 600, whiteSpace: 'nowrap' }}>Subtotal</th>
                                </tr>
                              </thead>
                              <tbody>
                                {pedido.produtos.map((prod, idx) => (
                                  <tr key={idx} style={{ borderTop: '1px solid #eee' }}>
                                    <td style={{ padding: '0.5rem 1rem' }}>
                                      ...
                                      {prod.codigo && (
                                        <div style={{ fontSize: '0.75rem', color: 'var(--gray)' }}>Cód: {prod.codigo}</div>
                                      )}
                                    </td>
...
export default function Clientes() {
  ...
  const [clienteSel,  setClienteSel]  = useState(null); // modal
  ...
  return (
    <>
      {/* Modal de pedidos */}
      {clienteSel && (
        <ModalPedidos cliente={clienteSel} onClose={() => setClienteSel(null)} />
      )}

      <div>
        <div className="header">
          <h1>Clientes</h1>
          <p>Lista completa de todas as fontes — clique em um cliente para ver os pedidos e produtos</p>
        </div>
              ...
              <div style={{ display: 'flex', gap: '0.25rem' }}>
                <div style={{ position: 'relative' }}>
                  ...
                  <input
                    ...
                    style={{ paddingLeft: '2.1rem', paddingRight: '0.75rem', paddingTop: '0.5rem', paddingBottom: '0.5rem', border: '1px solid var(--border)', borderRadius: 8, fontSize: '0.875rem', width: 210 }}
                  />
                </div>
                  ...
                  {filtrados.length === 0 ? (
                    ...
                    filtrados.map((c, i) => (
                      <tr
                        ...
                        title="Clique para ver pedidos e produtos"
                      >
                        <td style={{ fontWeight: 600 }}>
                          <span style={{ display: 'flex', alignItems: 'center', gap: '0.4rem' }}>
                            {c.nome}
                            <ShoppingBag size={13} color="var(--primary)" style={{ opacity: 0.5, flexShrink: 0 }} />
                          </span>
                        </td>
                        <td style={{ fontSize: '0.85rem' }}>
                          {c.telefone
                            ? <span style={{ display: 'flex', alignItems: 'center', gap: '0.3rem' }}><Phone size={13} color="var(--gray)" />{c.telefone}</span>
